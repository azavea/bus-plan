...
# workaround for IPython weirdness
...
import post_processing.map_solver as ms
# import post_processing.student_ride_times as srt
import post_processing.drive_distances as dr
...
class BusPlan():
    ...
    def __init__(self, plan_dir, total_students=1056, cost_matrix=None):
        """Create BusPlan from router output and student assignment output"""
        # parameters
        self.plan_dir = plan_dir
        self.route_csv = os.path.join(plan_dir, 'OUTPUT_router.csv')
        ...
        os.chdir(plan_dir)  # switch to plan directory
        self.ride_times_df = pd.read_csv(
            'OUTPUT_analysis_student_ride_times.csv')
        self.plan = pd.read_csv('OUTPUT_router.csv')
        self.routed_students = self.get_routed_students()
        self.student_ride_times = pd.read_csv(
            'OUTPUT_analysis_student_ride_times.csv')
        ...
        if (cost_matrix is not None):
            self.drive_distances = dr.route_distances(
                self.route_csv, cost_matrix)
        else:
    ...
    def get_walk_threshold(self):
        ...
        tail = os.path.split(self.route_csv)[1]
        s = tail[-9:-6]
        try:
            return float(s)
        except ValueError:
